...
import type { PaymentSample } from '../features/sampler/types'
...
export function exportBadActorIds(payments: PaymentSample[]) {
  ...
  const header = ['actor_id', 'value', 'payment_id', 'message_id'].join(',')
  const csvRows = rows.map((row) =>
    [row.actor_id, row.value, row.payment_id, row.message_id]
      .map((value) => `"${String(value).replace(/"/g, '""')}"`)
      .join(','),
  )
